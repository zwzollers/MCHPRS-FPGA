...
pub fn generate_verilog(graph: &Graph, path: &str, size: &super::schem::Size, ctx: &mut super::FPGABackend) 
{
    ...
    for node in &graph.nodes
    {
        ...
        match &node.block
        {
            ...
            NodeType::Output =>
            {
                ctx.outputs.push(node.pos.to_BlockPos());
...
pub fn get_inputs_str (node: &Node, graph: &Graph, ty: LinkType, size: &super::schem::Size) -> String
{
