...
impl<'a> Graph<'a>
{
    ...
    pub fn get_context (&self, outputs: &mut Vec<BlockPos>, inputs: &mut Vec<BlockPos>)
    {
        for node in &self.nodes
        {
            match node.block
            {
                NodeType::Output => outputs.push(BlockPos{x:node.pos.x as i32 + 108, y:node.pos.y as i32 + 7, z:node.pos.z as i32 + 143}),
                NodeType::Input {state:_} => inputs.push(BlockPos{x:node.pos.x as i32  + 108, y:node.pos.y as i32 + 7, z:node.pos.z as i32  + 143}),
                _ => ()
            }
        }
    }
...
impl Pos
{
    pub fn to_BlockPos(&self) -> BlockPos
    {
        BlockPos{x:self.x as i32 + 108, y:self.y as i32 + 7, z:self.z as i32 + 143}
    }
